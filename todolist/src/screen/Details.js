...
const Details = ({route, navigation}) => {
  ...
  useEffect(() => {
    // Pastikan id tersedia sebelum memanggil getDetail()
    if (id) {
      getDetail();
    }
  }, [id]);
    ...
    <View style={styles.container}>
      ...
      <View>
        ...
        <View style={styles.body}>
          ...
          <Text style={styles.body2}>{detail && detail.tanggal !== undefined ? detail.tanggal : 'Data tidak tersedia'}</Text>
        </View>
        <View style={styles.body}>
          <Text style={styles.body1}>Status :</Text>
          <Text style={styles.body2}>{detail && detail.status !== undefined ? detail.status : 'Data tidak tersedia'}</Text>
        </View>
        <View style={styles.body}>
          <Text style={styles.body1}>Kegiatan :</Text>
          <Text style={styles.body2}>{detail && detail.isi !== undefined ? detail.isi : 'Data tidak tersedia'}</Text>
        </View>
      </View>
      <TouchableOpacity
        style={styles.btn}
